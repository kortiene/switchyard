...
2. Re-run the landed #75 boundary on a compatible live host: durable auth remains in
   HOME/CODEX_HOME, while `--ignore-user-config` plus forced app/plugin disables close
   the prior connector path. The hermetic two-process metadata probe passes.
